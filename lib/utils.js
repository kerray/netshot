...
if (os.type().indexOf("Windows") != -1) {
	electroshotPath = path.join(__dirname, "..", "node_modules", "electroshot", "bin", "electroshot.js");
} else {
	electroshotPath = path.join(__dirname, "..", "node_modules", ".bin", "electroshot");
}
...
function runElectroshot(args, callback) {
	...
	if (os.type().indexOf("Windows") != -1) {
		args.unshift(electroshotPath);
	     var process = child_process.spawn("node", args);
	} else {
	     process = child_process.spawn(electroshotPath, args);
	}
